...
@author: Jonas Beuchert, modified by Ha My Nguyen
"""
...
def worker(data, experiment, mode):
    ...
    all_error = []
    all_time = []
    all_neu_pos = np.zeros([len(filenames), 3], dtype = float)
    ...
    for idx, filename in enumerate(filenames):
        ...
        latitude_estimates, longitude_estimates, _, _ \
        ...
        all_neu_pos[idx] = np.array([err_east, err_north, err_height])
        all_error.append(err)

    return {"error": all_error, "time": all_time, "position": all_neu_pos}
